...
| survivor-blind cover localization | `cover_lidl_probe.py` | glue local certificates by COVER not intersection: S_i = g_{i,0}+g_{i,1} survives the witness without knowing the bit (correct-side annihilator nonzero, wrong-side zero by the soundness control), and x_i = g_{i,1}/(g_{i,0}+g_{i,1}). Verified identity. Two design traps caught: u is not idempotent (dropping u^2 invents a spurious unit ideal) and the unit-ideal/wrong-bit soundness gate. Result: generic instances are sound (S_i all survive) but kappa(cover) EQUALS the plain Macaulay degree; the selector control breaks (the variable s has only a Boolean-trivial annihilator, so S_s=0 forces the unit ideal) | cover-localized quotient persistence: the cover localizers are inert in the x-sector (localizing back recovers g_{i,0}x_i+(x_i-1)g_{i,1} in I, already in I), so they add no constraint and do not collapse the quotient below the plain degree |
