"""Trick Room v2 — PyO3 native Rust substrate on Two-Room.

v1 (rust_model.py) used JSON-RPC stdin/stdout to the Rust subprocess — correct
result but 3,615 ms/ep due to serialisation.  v2 calls the same physics via
a PyO3 cdylib (tworoom_substrate) installed into this venv.  No subprocess,
no serialisation — Rust runs in-process with rayon batch parallelism.

Expected result: same 100% success, wall-clock approaching the numpy v0 (~91 ms/ep).

Build the extension first (once):
...
def main(num_eval: int = 50, seed: int = 42,
         out: str = 'results/headline_two_room_pyo3_substrate.json') -> dict:

    model = Pyo3SubstrateCostModel()
    print('[v2] Pyo3SubstrateCostModel loaded (native Rust, in-process)')
    ...
    print(f'[v2] running {num_eval} episodes (seed={seed})...')
    t0 = time.time()
    results = world.evaluate(episodes=num_eval, seed=seed)
    elapsed = time.time() - t0

    summary = {
        'label': 'v2_pyo3_substrate',
        'env': 'swm/TwoRoom-v1',
        'bridge': 'pyo3_native_cdylib',
        'config': {
            'num_eval': num_eval,
            'seed': seed,
    ...
    print('=' * 64)
    print(' TRICK ROOM v2 — PyO3 Native Substrate (Two-Room)')
    print('=' * 64)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out',      type=str,
                        default='results/headline_two_room_pyo3_substrate.json')
